...
public interface DiaryPostsRepository extends CrudRepository<DiaryPosts, Integer> {
    ...
    List<DiaryPosts> datenothappened();

    @Query("SELECT d FROM DiaryPosts d WHERE d.date BETWEEN ?1 AND ?2")
    List<DiaryPosts> datebetween(LocalDate startDate, LocalDate endDate);
